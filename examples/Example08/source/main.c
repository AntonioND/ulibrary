...
#define shadowedPrintf(x, y, color, format...) ({ \
        char __str[1000]; \
...
int main(int argc, char *argv[])
{
    ...
    // Note the Render argument: this function will be called from within
    // ulMessageBoxEx() to render the background image!
    nSystem = ulMessageBoxEx("Please select the system you want to use for loading files.",
                             "Hello", 3, buttons, NULL, 0);
    ...
    switch (nSystem)
    {
        ...
        case 2:
            // Try to init NitroFS
            ulDebug("Initializing NitroFS. If it's not found, verify that you "
                    "have a loader with argv support or that your emulator is working.\n");
            if (ulInitNitroFs() > 0)
                fsInited = 1;
            break;
    }
    ...
    ulSetTransparentColor(RGB15(31, 0, 31));
    {
        ...
        if (fsInited)
        {
            // By default all file loading functions will be using NitroFS or
            // FAT because we called that functions.
            imgBack = ulLoadImageFileJPG("/back.jpg", 0, UL_IN_VRAM, UL_PF_5551);
            if (imgBack)
            {
                // No matter its size, the image should be stretched (or
                // shrinked) to take the entire screen
            ...
            else
            {
                ulDebug("Image could not be loaded!\n"
                        "Maybe you didn't put a back.jpg image in the root of "
                        "your file system or the image is too big to be loaded.\n");
            }
        }

        // Now several sources exist, we must select which one we will load from
        // before doing it!
    ...
    while (1)
    {
        ...
        // If the background image has been loaded successfully, then draw it,
        ...
        {
            ulDrawGradientRect(0, 0, 256, 192, RGB15(0, 0, 16), RGB15(0, 0, 16),
                               RGB15(0, 16, 24), RGB15(0, 16, 24));
        }
