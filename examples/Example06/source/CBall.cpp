...
// These variables are shared between all instances, only one image is needed in
// memory at a time to draw every ball ;)
...
CBall::CBall(int x, int y, int angle, int speed)
{
    ...
    {
        // We load our ball image. Note that the format is PAL2 (4 colors). It's
        // enough because our image is nearly monochrome.
        this->imgBall = ulLoadImageFilePNG((const char*)ball_png, (int)ball_png_size,
                                           UL_IN_VRAM, UL_PF_PAL2);
    }
    this->nInstances++;

    // Initialize the class members - positions are passed as integer values,
    // convert them to fixed point values
    this->x = FIX(x);
    this->y = FIX(y);
    this->angle = angle;
    this->speed = speed;
