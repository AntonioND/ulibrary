/// @file glWrapper.h
///
/// @brief Wrapper for the default libnds VideoGL utility library.
///
/// Implements namely a dynamic texture system, which allows you to create and
/// remove textures / palettes into VRAM as you want.
...
///
/// Wrapper for the default libnds VideoGL utility library. Implements namely a
/// dynamic texture system, which allows you to create and remove textures /
/// palettes into VRAM as you want. It's used for internal purposes but you can
/// still use it if you want, however I can't guarantee full forward
/// compatibility for some "hacky" functions.
///
/// @{
...
#define ulGetTextureAddress(texID) \
        ((void*)(((ulTextureParams[texID] & 0xFFFF) << 3) + ul_texVramBase))
...
/// Changes the VRAM allocation parameters for textures.
///
/// By default, it is allocated to bank A and is 128 kB wide.
///
/// @param activeBanks
///     Banks that will be allocated to texture memory. For a description of
///     possible choices, see #UL_BANKS.  You can OR them to allocate several
///     banks. Usually UL_BANK_A | UL_BANK_B.
/// @param baseAddr
///     Video memory base address. Usually (void*)0x06800000. If you select
///     UL_BANK_B only, it will be 0x06820000 and so on.
/// @param totalSize
///     Total video memory size. For one bank (UL_BANK_A) it's 128 << 10, for
///     two banks (UL_BANK_A | UL_BANK_B) it's 256 << 10, and so on. << 10
///     multiplies a number by 1024, meaning kilobytes. It should be a multiple
///     of 16.
///
/// @return
...
///   the total memory is used, there may be not any space free at the end
///   because of memory fragmentation.
/// - The total size will be aligned to 16 manually if it's not, rounded to the
///   next 16 factor (above, never below).
///
/// Here are two lines: the first one is the default, the second one is the
/// recommended for accessing the 256 kB of texture memory.
/// ```
...
/// Same as ulSetTexVramParameters() but for palettes.
///
/// Default is:
/// ```
...
/// 1) sizeX and sizeY parameter specifies the texture size IN PIXELS, unlike in
///    libnds. If their size is not a power of two:
///
///    - The width will be rounded to the next power of two (ex: 100 => 128)
///    - The height will not be rounded, except if ul_optimizePaletteSize is 0,
///      but the texture size supplied to the GPU will be rounded to the next
///      power of two so that it accepts it.
///
...
///
/// This is an awful hack, and will REPLACE the currently active texture in
/// VideoGL. You should not use both systems at once, this hack is only designed
/// so that you can select a texture and use VideoGL commands that rely on the
/// active texture.
...
/// Returns the total number of bytes of texture palette memory (used +
/// remaining).
int ulGetTexPalTotalMemory();

/// Returns the number of vertices available in the vertex memory (internal
/// to the GPU).
///
/// Each vertex counts as 1 unit, so a quad will take 4 vertices in memory, and
