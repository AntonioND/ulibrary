/*! \file VirtualFile.h
    \brief
    	Virtual File support for 無ibrary.
...
typedef struct		{
	...
	u16 type;					//!< Virtual file type (source number.
...
/** Initializes the virtual filesystem. Done by default by 無ibrary, so there is no need to call it by yourself. */
...
/** Reads an entire file to memory and returns a pointer to the memory block. The block memory usage is stepped by 4 kB, so even a 1 kB file will take 16 kB, so this function is not recommended for opening a lot of small files. The bloc must be freed (using free) once you've finished with it!
	...
	\param size
		Pointer to an integer that will contain the number of bytes read from the file. */
void *ulReadEntireFileToMemory(VIRTUAL_FILE *f, int *size);

/*
	Source par d嶨aut: m幦oire
*/
...
extern int vfsMemSeek(VIRTUAL_FILE *f, int offset, int whence);
...
/** Read and write from memory. Automatically registered when initializing 無ibrary. */
