...
UL_IMAGE *ulLoadImagePNG(VIRTUAL_FILE *f, int location, int pixelFormat)
{
    ...
    png_read_png(pPngStruct, pPngInfo, PNG_TRANSFORM_STRIP_16 |
...
/*
    if (pPngInfo->num_trans > 0)
    {
        ...
        {
            transpActive = 1;
            theTransparentColor = ((pPngInfo->trans_values.red) |
                                  ((pPngInfo->trans_values.green) << 8) |
                                  ((pPngInfo->trans_values.blue) << 16));
            }
    }
*/
    ...
    if (img)
    {
        ...
        {
            // !!! Note qu'il faudrait aussi prendre en compte num_trans pour la
            ...
            if (img->palette && !dynamicPaletteRequired)
            {
                ...
                // Nous devons trouver la couleur transparente (color key)
                // parmis toutes les couleurs de la palette
                ...
                else
                {
                    ...
                    while (--i >= 0)
                    {
                        ...
                        {
                            ((u16*)img->palette)[0] = RGB15(palette[i].red >> 3,
                                                            palette[i].green >> 3,
                                                            palette[i].blue >> 3);
                        }
                        else
...
*/
                ...
                if (dynamicPaletteRequired && img->palette)
                {
                    ...
                    {
                        // Parcourt la palette pour voir si elle se trouve dans
                        // la palette
