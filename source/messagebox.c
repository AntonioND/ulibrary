...
void _drawButton(int x0, int y0, int x1, int y1, const char *text, int highlight)
{
    ...
    if (highlight)
    {
        ulDrawGradientRect(x0 + 1, y0 + 1, x1 - 1, y1 - 1, RGB15(18, 9, 9),
                           RGB15(15, 7, 7), RGB15(15, 7, 7), RGB15(12, 6, 6));
    }
    else
    {
        ulDrawGradientRect(x0 + 1, y0 + 1, x1 - 1, y1 - 1, RGB15(18, 18, 18),
                           RGB15(15, 15, 15), RGB15(15, 15, 15), RGB15(12, 12, 12));
    }
                   ...
                   UL_MSGBOX_BUTTON *buttons, void (*fnRender)(), u32 reserved)
{
    ...
    const UL_COLOR backColor0 = RGB15(103 >> 3, 107 >> 3, 115 >> 3);
    const UL_COLOR backColor1 = RGB15(73 >> 3, 77 >> 3, 85 >> 3);
    const UL_COLOR backColor2 = RGB15(43 >> 3, 47 >> 3, 55 >> 3);
    const UL_COLOR title1 = RGB15(0 >> 3, 0 >> 3, 128 >> 3);
    const UL_COLOR title2 = RGB15(16 >> 3, 132 >> 3, 208 >> 3);
    ...
    while (returnButton == -1)
    {
        ...
        ulDrawTextBox((x0 + x1 - textWidth) >> 1, y0 + 3, x1 - 2,
                      y0 + titleHeight, title, 0);
        ...
        for (i = 0; i < nbButtons; i++)
        {
            ...
            _drawButton(x, buttonY, x + buttonMaxWidth, buttonY + buttonHeight,
                        buttons[i].text, highlight);
...
#ifdef UL_MSGBOX_TOUCHPAD_SUPPORT
        // Le code de gestion du touchpad ne fonctionne que si le touchpad est
        // effectivement actif...
        if (ulGetMainLcd() == 0)
        {
            ...
            for (i = 0; i < nbButtons; i++)
            {
                ...
                if (ul_keys.touch.x >= x && ul_keys.touch.y >= buttonY
                    && ul_keys.touch.x <= x + buttonMaxWidth
                    && ul_keys.touch.y <= buttonY + buttonHeight)
                {
                    buttonActive = i;
                }
                x += buttonMaxWidth + buttonSpace;
