...
void ul_splash1_drawBackText()
{
    ...
    ulDrawString((SCREEN_WIDTH - ulGetStringWidth(string)) / 2,
                 SCREEN_HEIGHT - ul_curFont->charHeight, string);

    string = "For more information, please visit";
    ulSetTextColor(RGB15(31, 31, 0));
    ulDrawString((SCREEN_WIDTH - ulGetStringWidth(string)) / 2,
                 SCREEN_HEIGHT - ul_curFont->charHeight * 2, string);
}
...
void ul_splash1_fillSecondScreen()
{
    ...
    {
        ulStartDrawing2D();
        ulDrawGradientRect(0, 0, 256, 192, RGB15(0, 0, 8), RGB15(0, 0, 8),
    ...
    // Captures to LCD bank C - 1<<24 to capture only 3D, not fading and so on
    REG_DISPCAPCNT = DCAP_BANK(2) | DCAP_ENABLE | DCAP_SIZE(3) | 1 << 24;
    swiWaitForVBlank();
...
/*
    ulInit(UL_INIT_ALL);
    ulInitGfx();
    // should have been done before. Nothing should be loaded in memory as this
    // splash uses about 125 kB of video memory (!) out of 128 kB.
*/
int ulShowSplashScreen1()
{
    UL_IMAGE *imgLogo = NULL, *imgLogoMask = NULL, *imgFrame = NULL;
    UL_IMAGE *imgCircle = NULL, *imgLogoSub = NULL;
    int success = 0;
    ...
    imgLogoSub = ulLoadImageFileGIF((const char*)ul_logo_base,
                                    (int)ul_logo_base_size, UL_IN_RAM, UL_PF_PAL5_A3);
    ...
    imgCircle = ulLoadImageFileGIF((const char*)ul_circle_mask,
                                   (int)ul_circle_mask_size, UL_IN_RAM, UL_PF_5551);
