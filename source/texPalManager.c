...
#define setBlockFree(i, free) \
        (ulTexPalBlocks[i].size = (ulTexPalBlocks[i].size & ((free) ? 0xffff : 0x7fff)) | ((free)? 0x8000 : 0))
#define setBlockSize(i, nsize) \
        (ulTexPalBlocks[i].size = (ulTexPalBlocks[i].size & ~0x7fff) | (nsize))

#define setBlockOffset(i, noffset) \
        (ulTexPalBlocks[i].offset = noffset)
    ...
    do
    {
        ...
        for (j = 0; j < ulTexPalBlocksNb - 1; j++)
        {
                ...
                (isBlockFree(j) && getBlockSize(j) == 0))
            {
                ...
                memmove(ulTexPalBlocks + j, ulTexPalBlocks + j + 1,
                        sizeof(UL_TEXPALBLOCK) * (ulTexPalBlocksNb - j - 1));
